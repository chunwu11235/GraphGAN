...
from pipeline import preprocessing, get_item_feature_columns, get_user_feature_columns, df2tensor, get_type_dict, \
    construct_feed_dict
from utils import data_iterator, progress_bar
from model import BilinearDecoder, LogisticsDecoder
...
def main(args):
    ...
    MODEL = BilinearDecoder if args.bilinear else LogisticsDecoder
    ...
    if args.use_gpu:
        with tf.device(assign_to_device('/gpu:{}'.format(0), ps_device='/cpu:0')):
            ...
            #
            # u_feature_placeholder_dict = {}
            # for k, v in user_type_dict.items(): u_feature_placeholder_dict[k] = tf.placeholder(v, shape = (None,)) placeholders['u_features'] = u_feature_placeholder_dict
            # placeholders['v_features'] = v_feature_placeholder_dict
            #
            
            model = MODEL(placeholders, model_params)
            ...
            for epoch in range(num_epoch):
                ...
                try:
                    while True:
                        ...
                        train_feed_dict = construct_feed_dict(placeholders, train_reviews, additional_info,
                                                              model_params)
                        ...
                        if model.global_step.eval() % args.summary_steps == 0:
                            ...
                            try:
                                while True:
                                    ...
                                    val_feed_dict = construct_feed_dict(placeholders, val_reviews, additional_info,
                                                                        model_params)
                                    ...
                                    progress_bar(val_total, num_val, 'Loss: %.3f | Acc: %.3f%% (%d/%d)' \
                                                 % (val_total_loss / val_total, 100. * val_total_accuracy / val_total,
                                                    val_total_accuracy, val_total))
                        ...
                        progress_bar(train_total, num_train, 'Loss: %.3f | Acc: %.3f%% (%d/%d)' \
                                     % (train_total_loss / train_total, 100. * train_total_accuracy / train_total,
                                        train_total_accuracy, train_total))
                except StopIteration:
                    pass
...
if __name__ == "__main__":
    ...
    parser.add_argument('--bilinear',default=True, type=bool, help="Whether using bilinear decoder or softmax")
